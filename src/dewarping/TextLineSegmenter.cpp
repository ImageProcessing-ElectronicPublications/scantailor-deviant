    ...
    TaskStatus const& status, DebugImages* dbg)
{
    ...
    Result result = processDownscaled(
                        downscaled_image, downscaled_crop_area, status, dbg
                    );
    ...
    TaskStatus const& status, DebugImages* dbg)
{
    ...
    std::pair<Grid<float>, Grid<uint8_t>> filterbank(
                                           textFilterBank(src, directions, sigmas, 6.f)
                                       );
...
{
    BinaryImage const binarized(binarizeGatos(image, QSize(9, 9)));
                                 ...
                                 ConnectivityMap& cmap, TaskStatus const& status, DebugImages* dbg)
{
    ...
    BinaryImage binarized(binarizeGatos(seed, QSize(9, 9)));
    seed = GrayImage();
